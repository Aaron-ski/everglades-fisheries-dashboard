...
export function defaultState(metadata: SourceMetadata): DashboardState {
  return {
    ...
    metric: "cpue",
    areaMode: "regions",
    selectedRegions: metadata.regions?.length ? [...metadata.regions] : [...fallbackRegions],
    selectedAreas: [],
...
export function stateFromUrl(metadata: SourceMetadata, search: string): DashboardState {
  ...
  const selectedRegions = parseList(params.get("regions")).filter((region) => defaults.selectedRegions.includes(region));
  ...
  return {
    ...
    selectedRegions: selectedRegions.length ? selectedRegions : defaults.selectedRegions,
